# Using code from DTU course 02460 (Advanced Machine Learning Spring) by Jes Frellsen, 2024 as template
...
def train(model, optimizer, data_loader, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        for x in data_iter:
            ...
            if epoch + 1 % 10 == 0:
                torch.save(model.state_dict(), f"{args.model}-{epoch+1}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('mode', type=str, default='train', choices=['train', 'sample', 'sample_save'],
    ...
    for i in range(num_transformations):
        ...
        scale_net = nn.Sequential(nn.Linear(D, num_hidden), nn.ReLU(), nn.BatchNorm1d(num_hidden),
                                  nn.Linear(num_hidden, num_hidden), nn.ReLU(), nn.BatchNorm1d(num_hidden),
                                  nn.Linear(num_hidden, D), nn.Tanh())

        translation_net = nn.Sequential(nn.Linear(D, num_hidden), nn.ReLU(), nn.BatchNorm1d(num_hidden),
                                        nn.Linear(num_hidden, num_hidden), nn.ReLU(), nn.BatchNorm1d(num_hidden),
